    ...
    <div className="px-[15%] w-screen flex flex-col bg-[#1F1E24] h-[150vh]">
      ...
      <div className="w-full flex "> 
        <div className="w-[20%]"> 
          ...
          <h1 className="text-lg text-zinc-400 font-semibold my-3">Birthday</h1>
          <h1 className=" text-zinc-400">{info.detail.birthday}</h1>

          <h1 className="text-lg text-zinc-400 font-semibold my-3">Deathday</h1>
          <h1 className=" text-zinc-400">{info.detail.deathday ? info.detail.deathday : "umm.... Still Alive"}</h1>

          <h1 className="text-lg text-zinc-400 font-semibold my-3">Place of Birth</h1>
          <h1 className=" text-zinc-400">{info.detail.place_of_birth}</h1>

          <h1 className="text-lg text-zinc-400 font-semibold my-3">Also Known As</h1>
          <h1 className=" text-zinc-400">{info.detail.also_known_as.join(", ")}</h1>

        </div>
        <div className="w-[80%] ml-[5%]">
          <h1 className=" text-zinc-400 text-6xl font-extrabold">{info.detail.name}</h1>
          
          <h1 className="text-xl text-zinc-400 font-semibold my-5">Overview</h1>
          <p className="text-zinc-400">{info.detail.biography}</p>
          
          <h1 className="text-lg text-zinc-400 font-semibold mt-5">Movies and TV Shows</h1>
          <HorizontalCard data = {info.combinedCredits.cast}/>
          <div className="w-full flex justify-between">
            <h1 className="text-lg text-zinc-400 font-semibold mt-5">Movies and TV Shows</h1>
            <DropDown title="category" options={["tv",'movie']} func={(e)=>setcategory(e.target.video)}/>
          </div>
        </div>
      </div>
    </div> 
